...
When the bot runs on a **different computer** than the player's game, use the
**Mingler Sync Windows app** with `/register link`. The inspectable PowerShell
package from `/register script` remains a supported alternative.

## Flow

1. In Discord (any server where The Mingler is installed): **`/register link`**
   - Accept terms if prompted.
   - Bot replies **ephemerally** with a private command containing a **one-time token** (15 minutes by default).

2. On the **game PC** (game open, toon logged in, Companion App Support ON):
   - Install/open [Mingler Sync](https://apps.microsoft.com/detail/9PP2WVDVMX9L).
   - Paste the entire private command from `/register link`.
...
Public app source and docs: [github.com/Wazzup101/The-Mingler](https://github.com/Wazzup101/The-Mingler) (`apps/MinglerSync/`, `scripts/register-sync-pack/`, `scripts/register-sync.ps1`, this file).
...
Example JSON snippets: [`docs/examples/register-sync-home-lan.config.json`](./examples/register-sync-home-lan.config.json), [`docs/examples/register-sync-internet.config.json`](./examples/register-sync-internet.config.json).
...
**POST** `/api/register/client-error`

- Accepts the same still-valid bearer token without consuming it.
- Accepts at most 8 KiB and rate-limits reports per authenticated user.
- Allowlisted fields: app phase/version and bounded error type/message.
- Never accepts or records the token in the body, raw Companion data, passwords, file paths, or device inventory.
- Returns `202` whether the authenticated report is newly delivered or rate-limited.
...
- **`/register link`** — private one-time command for Mingler Sync.
- **`/register script`** — download the inspectable PowerShell fallback (ephemeral).
- **`/register status`** — last push sync.

## Security notes

- Never share your token; it binds to your Discord user only.
- Install the Windows app only from the Microsoft Store or inspect/build the public source. Modified binaries could misuse pasted tokens or Companion data.
- Unexpected app faults may send a bounded diagnostic to a private operator channel. The report excludes the token and raw Companion payload. Network/service failures may prevent delivery.
- No software is guaranteed vulnerability-free. Keep Windows and Microsoft Defender current and use `/tickets` for suspicious behavior.
